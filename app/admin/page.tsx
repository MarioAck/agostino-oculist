  ...
  const fetchItems = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        alert(`Impossibile caricare gli articoli: ${errorData.error || "Errore sconosciuto"}`);
        return;
      }
      ...
      alert("Impossibile caricare gli articoli. Controlla la console per i dettagli.");
    }
  };
    ...
    if (files.length > 0) {
      ...
      for (const file of files) {
        try {
          ...
          alert(`Impossibile caricare ${file.name}: ${error instanceof Error ? error.message : 'Errore sconosciuto'}`);
          // Remove failed item
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (hasUnuploadedFiles) {
        alert("Attendi che tutte le immagini vengano caricate");
        return;
      }
      ...
      alert(`Impossibile salvare l'articolo: ${error instanceof Error ? error.message : 'Riprova.'}`);
    }
  };
      ...
      <header className="relative z-10 w-full px-4 md:px-8 lg:px-24 py-6 md:py-8 border-b border-[#e8dcc4]/10">
        ...
        <div>
          ...
          <p className="text-base md:text-lg lg:text-xl text-[#e8dcc4]/80 tracking-wide">
            Gestisci il tuo inventario di occhiali
          </p>
        </div>
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-4 md:px-8 lg:px-24 py-8 md:py-12 lg:py-16 flex-grow">
          ...
          {(isAddingNew || editingItem) && (
            <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-xs text-[#e8dcc4]/60 mb-3">
                    Clicca il pulsante + per aggiungere immagini. Le immagini vengono caricate immediatamente e automaticamente ottimizzate a 2000x2000px WebP di alta qualità (fino a ~5MB). Caricamento massimo 50MB. Trascina per riordinare.
                  </p>
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 pt-2 md:pt-4">
                ...
                >
                  {editingItem ? "AGGIORNA ARTICOLO" : "AGGIUNGI ARTICOLO"}
                </button>
                <button
                  type="button"
                  onClick={resetForm}
              ...
              >
                ...
                <div className="p-4 md:p-6">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 md:gap-3">
                    <Link
                      href={`/item/${item.id}`}
                      className="flex-1 border-2 border-[#e8dcc4]/50 text-[#e8dcc4] px-3 md:px-4 py-2 rounded transition-all duration-200 font-semibold text-sm md:text-base hover:bg-[#e8dcc4]/10 hover:border-[#e8dcc4] flex items-center justify-center"
                      title="Visualizza"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 border-2 border-[#e8dcc4] text-[#e8dcc4] px-3 md:px-4 py-2 rounded transition-all duration-200 font-semibold text-sm md:text-base hover:bg-[#e8dcc4] hover:text-[#2d1810] flex items-center justify-center"
                      title="Modifica"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button
                      ...
                      className="flex-1 border-2 border-red-700 text-red-400 px-3 md:px-4 py-2 rounded transition-all duration-200 font-semibold text-sm md:text-base hover:bg-red-700 hover:text-white flex items-center justify-center"
                      title="Elimina"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {bestSellers.length === 0 && (
            <div className="text-center py-16 bg-gradient-to-br from-[#1a1310]/80 to-[#2d1810]/80 backdrop-blur-sm rounded-lg shadow-lg border border-[#e8dcc4]/10">
              <p className="text-[#e8dcc4]/70 text-lg">
                Nessun best seller ancora. Aggiungi il tuo primo articolo!
              </p>
            </div>
          )}
        </div>
        ...
        <div>
          ...
          {saleItems.length === 0 && (
            <div className="text-center py-16 bg-gradient-to-br from-[#1a1310]/80 to-[#2d1810]/80 backdrop-blur-sm rounded-lg shadow-lg border border-[#e8dcc4]/10">
              <p className="text-[#e8dcc4]/70 text-lg">
                Nessun articolo in saldo ancora. Aggiungi il tuo primo articolo!
              </p>
            </div>
          )}
        </div>
